import { GENERALS, BUILDINGS, EXPLORATION_TYPES, RESOURCES, UNIT_TYPES, ABILITIES, STORE_PACKS } from './data.js';
...
export class UIManager {
    ...
    showGameUI() {
        ...
        // Store Button (Shop Icon)
        const storeBtn = document.createElement('button');
        storeBtn.className = 'hud-btn';
        storeBtn.innerHTML = '<i class="ra ra-gem"></i>';
        storeBtn.title = t('ui.store');
        storeBtn.onclick = () => this.showStoreUI();
        hudButtons.appendChild(storeBtn);
        ...
        // Add click listener for Diamond conversion
        resourcesBar.onclick = (e) => {
            const box = e.target.closest('.resource-box');
            if (box && box.dataset.id === 'diamonds') {
                if (confirm(t('ui.convert_diamond'))) {
                    if (!this.gameManager.convertDiamond()) {
    ...
    showStoreUI() {
        ...
        overlay.style.zIndex = '3000'; // High z-index

        const content = document.createElement('div');
        content.className = 'modal-content store-modal';
        content.style.width = '800px';
        content.style.height = '600px';
        content.style.display = 'flex';
        content.style.flexDirection = 'column';
        content.style.padding = '0'; // Custom padding layout
        ...
        title.innerHTML = `<i class="ra ra-gem"></i> ${t('ui.store')}`;
        ...
        sidebar.style.width = '70px'; // Compact width
        sidebar.style.background = '#2a2a2a';
        sidebar.style.borderRight = '1px solid #444';
        sidebar.style.padding = '10px 0'; // Vertical padding only
        sidebar.style.display = 'flex';
        sidebar.style.flexDirection = 'column';
        sidebar.style.gap = '15px';
        sidebar.style.alignItems = 'center'; // Center icons
        ...
        mainArea.style.gridTemplateColumns = 'repeat(auto-fill, minmax(180px, 1fr))';
        mainArea.style.gap = '20px';
        mainArea.style.alignContent = 'start';

        // Render Function
        const renderCategory = (category) => {
            mainArea.innerHTML = '';

            // Highlight sidebar (Update styles)
            updateActiveButton(category);

            if (category === 'magic' || category === 'skills') {
                // Render Spells/Skills
                Object.values(ABILITIES).forEach(ability => {
                    // Filter by type
                    if (category === 'magic' && ability.type !== 'spell') return;
                    if (category === 'skills' && ability.type !== 'skill') return;

                    const isUnlocked = this.gameManager.unlockedAbilities.includes(ability.id);
                    const item = document.createElement('div');
                    item.className = 'store-item';
                    item.style.border = isUnlocked ? '2px solid lime' : '1px solid #666';
                    item.style.padding = '15px';
                    item.style.borderRadius = '8px';
                    item.style.background = '#333';
                    item.style.textAlign = 'center';
                    item.style.opacity = isUnlocked ? '0.7' : '1';

                    const icon = document.createElement('div');
                    icon.style.fontSize = '3em';
                    icon.style.color = '#' + ability.color.toString(16);
                    icon.innerHTML = `<i class="${ability.icon}"></i>`;
                    item.appendChild(icon);

                    const name = document.createElement('h3');
                    name.textContent = t(ability.name);
                    name.style.margin = '10px 0 5px 0';
                    item.appendChild(name);

                    const desc = document.createElement('p');
                    desc.textContent = t(ability.desc);
                    ...
                    if (isUnlocked) {
                        ...
                        actionBtn.textContent = t('ui.unlocked');
                        actionBtn.className = 'btn';
                        ...
                        let costText = '';
                        // Determine cost
                        if (cost.diamonds) costText = `${cost.diamonds} Diamonds`;
                        else if (cost.solidi) costText = `${cost.solidi} Solidi`;
                        else costText = 'Free';

                        actionBtn.textContent = `${t('ui.unlock')} (${costText})`;
                        actionBtn.className = 'btn primary-btn';
                        actionBtn.onclick = () => {
                            if (this.gameManager.unlockAbility(ability.id)) {
                                this.showNotification(t('ui.purchase_success'), 'success');
                                renderCategory(category); // Re-render
                            } else {
                                this.showNotification(t('ui.not_enough_resources'), 'error');
                            }
                        };
                    }
                ...
                // Render Packs (Diamonds, Resources)
                const packs = STORE_PACKS[category] || [];
                packs.forEach(pack => {
                    const item = document.createElement('div');
                    item.className = 'store-item';
                    ...
                    const icon = document.createElement('div');
                    icon.style.fontSize = '3em';
                    icon.style.color = category === 'diamonds' ? 'cyan' : 'gold';
                    icon.innerHTML = `<i class="${pack.icon}"></i>`;
                    item.appendChild(icon);

                    const name = document.createElement('h3');
                    name.textContent = t(pack.name);
                    name.style.margin = '10px 0 5px 0';
                    item.appendChild(name);

                    // Reward Text
                    const rewardText = Object.entries(pack.reward).map(([k, v]) => `+${v} ${t('res.' + k)}`).join(', ');
                    const rDiv = document.createElement('div');
                    rDiv.textContent = rewardText;
                    rDiv.style.color = '#8fbc8f';
                    ...
                    // Cost Display
                    let costDisplay = pack.cost;
                    if (typeof pack.cost === 'object') {
                        costDisplay = Object.entries(pack.cost).map(([k, v]) => `${v} ${t('res.' + k)}`).join(', ');
                    }

                    actionBtn.textContent = `${t('ui.buy')} (${costDisplay})`;
                    actionBtn.onclick = () => {
                        if (this.gameManager.buyStoreItem(pack.id, category)) {
                            this.showNotification(t('ui.purchase_success'), 'success');
                        } else {
                            this.showNotification(t('ui.not_enough_resources'), 'error');
                        }
        ...
        // Tabs
        const categories = [
            { id: 'diamonds', name: 'ui.treasury', icon: 'ra ra-diamond' },
            { id: 'resources', name: 'ui.armory', icon: 'ra ra-anvil' },
            { id: 'magic', name: 'ui.magic', icon: 'ra ra-burning-meteor' },
            { id: 'skills', name: 'ui.skills', icon: 'ra ra-muscle-up' }
        ];

        categories.forEach(cat => {
            const btn = document.createElement('button');
            btn.innerHTML = `<i class="${cat.icon}"></i>`;
            btn.title = t(cat.name); // Tooltip
            btn.dataset.cat = cat.id;
            btn.className = 'sidebar-btn';

            // Inline styles for compaction
            ...
            btn.onclick = () => renderCategory(cat.id);
            sidebar.appendChild(btn);
        });

        // Add active style handler in renderCategory or here
        // We need a way to style the active button.
        // Let's add a style block for sidebar-btn if not exists, or handle inline.
        // For now inline active handling in renderCategory is best.

        // Update active handling in renderCategory
        const updateActiveButton = (activeId) => {
            Array.from(sidebar.children).forEach(b => {
                if (b.dataset.cat === activeId) {
    ...
    showNotification(message, type = 'info') {
        ...
        // Style (inline for now, or move to CSS)
        notif.style.position = 'fixed';
        notif.style.bottom = '20px';
        notif.style.left = '50%';
        notif.style.transform = 'translateX(-50%)';
        notif.style.backgroundColor = type === 'error' ? 'rgba(200, 50, 50, 0.9)' : 'rgba(50, 50, 50, 0.9)';
    ...
    updateGameHUD() {
        ...
        if (resDisplay) {
            ...
            // Define icons/labels
            const resData = [
                { id: 'gems', label: t('res.gems'), icon: 'ra ra-crystals icon-gems', val: Math.floor(r.gems || 0) },
                { id: 'diamonds', label: t('res.diamonds'), icon: 'ra ra-diamond icon-diamonds', val: Math.floor(r.diamonds || 0) },
                ...
                { id: 'weapons', label: t('res.weapons'), icon: 'ra ra-sword icon-weapons', val: Math.floor(r.weapons || 0) },
                { id: 'armor', label: t('res.armor'), icon: 'ra ra-vest icon-armor', val: Math.floor(r.armor || 0) },
            ...
            resDisplay.innerHTML = resData.map(res => `
                <div class="resource-box" data-id="${res.id}">
    ...
    showBuildingUpgradeModal(buildingId) {
        ...
        const renderModalContent = () => {
                    ...
                    <div style="margin: 20px 0; border-top: 1px solid #444; padding-top: 10px;">
                        ...
                        <div style="display: flex; gap: 10px; justify-content: center; margin-bottom: 10px;">
                            <div style="text-align: center;">
                                <div style="font-size: 2em;"><i class="ra ra-sword"></i></div>
                                <div>${t('res.weapons')}</div>
                                <div style="font-size: 0.8em; color: #aaa;">${t('ui.craft_cost')}: 10 ${t('res.iron')}</div>
                                <button id="craft-weapon" class="btn primary-btn" ${this.gameManager.resources.iron < 10 ? 'disabled' : ''}>${t('ui.craft')}</button>
                            </div>
                            <div style="text-align: center;">
                                <div style="font-size: 2em;"><i class="ra ra-vest"></i></div>
                                <div>${t('res.armor')}</div>
                                <div style="font-size: 0.8em; color: #aaa;">${t('ui.craft_cost')}: 20 ${t('res.iron')}</div>
                                <button id="craft-armor" class="btn primary-btn" ${this.gameManager.resources.iron < 20 ? 'disabled' : ''}>${t('ui.craft')}</button>
                            </div>
                        </div>
                    </div>
                ` : ''}
            ...
            if (building.type === 'smithy') {
                const craftWeaponBtn = overlay.querySelector('#craft-weapon');
                if (craftWeaponBtn) {
                    craftWeaponBtn.onclick = () => {
                        if (this.gameManager.craftItem('weapons')) {
                            this.showNotification(t('ui.craft_success'), 'success');
                            renderModalContent();
                        } else {
                            this.showNotification(t('ui.not_enough_resources'), 'error');
                        }
                ...
                const craftArmorBtn = overlay.querySelector('#craft-armor');
                if (craftArmorBtn) {
                    craftArmorBtn.onclick = () => {
                        if (this.gameManager.craftItem('armor')) {
                            this.showNotification(t('ui.craft_success'), 'success');
                            renderModalContent();
                        } else {
                            this.showNotification(t('ui.not_enough_resources'), 'error');
    ...
    showBattleUI() {
        console.log("UIManager: showBattleUI called");
        console.log("Current Mode:", this.gameManager.sceneManager.mode);
        console.log("Unlocked Abilities:", this.gameManager.unlockedAbilities);
        ...
        battlePanel.style.zIndex = '1000'; // Ensure it's on top

        battlePanel.style.zIndex = '1000'; // Ensure it's on top

        console.log("Abilities available in DATA:", Object.keys(ABILITIES));
        let renderCount = 0;

        Object.values(ABILITIES).forEach(ability => {
            // Check if unlocked
            if (!this.gameManager.unlockedAbilities.includes(ability.id)) {
                console.log(`Ability ${ability.id} is NOT unlocked.`);
                return;
            }
            console.log(`Rendering Ability Button: ${ability.id}`);
            renderCount++;
            ...
            btn.innerHTML = `<i class="${ability.icon}"></i>`;
            btn.title = `${t(ability.name)}: ${t(ability.desc)}`;
            btn.style.width = '100%';
            btn.style.height = '100%';
            btn.style.borderRadius = '50%';
            btn.style.border = `2px solid #${ability.color.toString(16)}`;
            ...
            btn.onclick = () => {
                const success = this.gameManager.sceneManager.activateAbility(ability.id);
                if (success) {
        ...
        console.log(`BattlePanel appended. Rendered ${renderCount} buttons.`);
        console.log("BattlePanel style:", battlePanel.style.cssText);
    ...
    updateBattleUI() {
        ...
        const abilityState = sm.abilityState || {};
        const now = Date.now();

        Object.values(ABILITIES).forEach(ability => {
            const overlay = document.getElementById(`cd-${ability.id}`);
            if (overlay) {
    ...
    renderShop(container) {
        Object.values(UNIT_TYPES).forEach(type => {
            ...
            // Handle complex cost object
            const cost = (typeof type.cost === 'object') ? type.cost : { solidi: type.cost };
            let costHtml = '';

            if (cost.solidi) costHtml += `<i class="ra ra-gold-bar"></i> ${cost.solidi} `;
            if (cost.weapons) costHtml += `<span style="color: #aaa; margin-left: 5px;"><i class="ra ra-sword"></i> ${cost.weapons}</span> `;
            if (cost.armor) costHtml += `<span style="color: #aaa; margin-left: 5px;"><i class="ra ra-vest"></i> ${cost.armor}</span> `;
            ...
            item.onclick = () => {
                const def = UNIT_TYPES[type.id];
                const cost = (typeof def.cost === 'object') ? def.cost : { solidi: def.cost };

                // Debug: Check affordable
                let canAfford = true;
                for (const [res, amount] of Object.entries(cost)) {
                    if ((this.gameManager.resources[res] || 0) < amount) {
                        canAfford = false;
                        break;
                    }
                }
                ...
                if (bought) {
                    ...
                    this.showNotification(t('ui.unit_recruited'), 'success'); // Temporary feedback
                } else {
                    this.showNotification("Army Grid Full!", 'error');
                }
            };
    ...
    startBattle() {
        ...
        // 4. Show Battle UI (Abilities & HUD)
        this.showBattleUI();
    }
    ...
    endBattle(victory, deadUnits = []) {
        // Cleanup Battle UI (Abilities, cooldowns, etc)
        this.hideBattleUI();
