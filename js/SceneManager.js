...
import { BUILDINGS, UNIT_TYPES, ABILITIES } from './data.js';
...
export class SceneManager {
    ...
    createCityEnvironment() {
        ...
        // Grid Helper - REMOVED as per user request
        // const size = 500; 
        // const divisions = 50;
    ...
    activateAbility(abilityId) {
        ...
        if (!this.abilityState) this.abilityState = {};
        const now = Date.now();

        if (this.abilityState[abilityId] && now < this.abilityState[abilityId].readyTime) {
        ...
        this.abilityState[abilityId] = { readyTime: now + ability.cooldown };

        // --- ABILITY EFFECTS ---

        const players = this.combatants.filter(c => c.side === 'player' && c.hp > 0);
        const enemies = this.combatants.filter(c => c.side === 'enemy' && c.hp > 0);

        if (abilityId === 'fireball' || abilityId === 'meteor') {
            if (enemies.length > 0) {
                const target = enemies[Math.floor(Math.random() * enemies.length)];
                const pos = target.mesh.position.clone();
                this.spawnProjectile(pos, ability, () => {
                    this.createExplosion(pos, ability.color, ability.radius || 5);
                    enemies.forEach(e => {
                        if (e.mesh.position.distanceTo(pos) <= ability.radius) {
                            e.hp -= ability.damage;
        ...
        } else if (abilityId === 'ice_nova') {
            if (enemies.length > 0) {
                // Freezes all enemies for now (or area?) - Let's do Global for simplicity/impact
                enemies.forEach(e => {
                    e.debuffs = e.debuffs || {};
                    e.debuffs.frozen = Date.now() + ability.duration;
                    e.material.emissive.setHex(ability.color);
                    e.flashTimer = 0; // Prevent flash overwrite
                });
            }

        } else if (abilityId === 'thunderbolt') {
            if (enemies.length > 0) {
                // Strongest Enemy
                const target = enemies.reduce((prev, curr) => (prev.hp > curr.hp) ? prev : curr);
                // Visual: Lightning
                this.createLightning(target.mesh.position, ability.color);
            ...
            players.forEach(p => {
                ...
                this.updateUnitVisuals(p);
                // Visual: Green upward particles?
                this.createParticles(p.mesh.position, ability.color);
            });

        } else if (abilityId === 'fury') {
            players.forEach(p => {
                p.buffs = p.buffs || {};
                p.buffs.fury = Date.now() + ability.duration;
                p.material.emissive.setHex(ability.color);
            });

        } else if (abilityId === 'shield_wall') {
            players.forEach(p => {
                p.buffs = p.buffs || {};
                p.buffs.shield = Date.now() + ability.duration;
                p.material.emissive.setHex(ability.color);
            });

        } else if (abilityId === 'haste') {
            players.forEach(p => {
                p.buffs = p.buffs || {};
                p.buffs.haste = Date.now() + ability.duration;
                p.material.emissive.setHex(ability.color);
            });
            ...
            enemies.forEach(e => {
                e.debuffs = e.debuffs || {};
                e.debuffs.weak = Date.now() + ability.duration;
                e.material.emissive.setHex(ability.color);
            });

        } else if (abilityId === 'whirlwind') {
            // Melee units deal damage around them
            players.forEach(p => {
                if (p.range < 5) { // Melee only check
                    this.createSpinEffect(p.mesh.position, ability.color);
                    enemies.forEach(e => {
                        if (p.mesh.position.distanceTo(e.mesh.position) <= ability.radius) {
                            e.hp -= ability.damage;
                            this.updateUnitVisuals(e);
                        }
                    });
    ...
    spawnProjectile(targetPos, ability, onHit) {
        const geo = new THREE.SphereGeometry(2);
        const mat = new THREE.MeshBasicMaterial({ color: ability.color });
        const projectile = new THREE.Mesh(geo, mat);
        projectile.position.set(targetPos.x, 50, targetPos.z);
    ...
    createLightning(pos, color) {
        // Simple cylinder flash
        const geo = new THREE.CylinderGeometry(0.5, 0.5, 50);
        const mat = new THREE.MeshBasicMaterial({ color: color });
        const mesh = new THREE.Mesh(geo, mat);
        mesh.position.set(pos.x, 25, pos.z);
        this.battleGroup.add(mesh);
        setTimeout(() => this.battleGroup.remove(mesh), 200);
    }

    createParticles(pos, color) {
        // Simple particles moving up
        const count = 5;
        for (let i = 0; i < count; i++) {
            ...
            this.battleGroup.add(m);

            const speed = 0.2 + Math.random() * 0.2;
            const animate = () => {
                m.position.y += speed;
                m.scale.multiplyScalar(0.95);
                if (m.scale.x > 0.1) requestAnimationFrame(animate);
                else this.battleGroup.remove(m);
            };
            animate();
        }
    }

    createSpinEffect(pos, color) {
        const geo = new THREE.TorusGeometry(4, 0.5, 8, 16);
        const mat = new THREE.MeshBasicMaterial({ color: color });
        const mesh = new THREE.Mesh(geo, mat);
        ...
        const animate = () => {
            ...
            if (mesh.scale.x < 3) requestAnimationFrame(animate);
            else this.battleGroup.remove(mesh);
        };
        animate();
    }

    createExplosion(pos, color, radius = 2) {
    ...
    updateBattle() {
        ...
        for (let i = 0; i < this.combatants.length; i++) {
            ...
            if (c.hp > 0) {
                ...
                // --- NEW BUFFS EXPIRE ---
                if (c.buffs && c.buffs.shield && now > c.buffs.shield) { delete c.buffs.shield; c.material.emissive.setHex(0x000000); }
                if (c.buffs && c.buffs.haste && now > c.buffs.haste) { delete c.buffs.haste; c.material.emissive.setHex(0x000000); }
                if (c.debuffs && c.debuffs.weak && now > c.debuffs.weak) { delete c.debuffs.weak; c.material.emissive.setHex(0x000000); }
        ...
        for (let i = 0; i < allAlive.length; i++) {
            ...
            if (unit.flashTimer > 0) {
                ...
                if (unit.flashTimer <= 0) {
                    ...
                    else if (unit.buffs && unit.buffs.shield) unit.material.emissive.setHex(0x0000ff);
                    else if (unit.buffs && unit.buffs.haste) unit.material.emissive.setHex(0x00ff88);
                    else if (unit.debuffs && unit.debuffs.frozen) unit.material.emissive.setHex(0x00ffff);
                    else if (unit.debuffs && unit.debuffs.weak) unit.material.emissive.setHex(0xffaa00);
            ...
            if (nearest) {
                ...
                if (realDist <= unit.range) {
                    ...
                    if (unit.cooldown <= 0) {
                        ...
                        // Handle Shield Wall Mitigation
                        if (nearest.buffs && nearest.buffs.shield) {
                            dmg *= 0.1; // 90% reduction
                        }
                        // Handle War Cry Weakness
                        if (unit.debuffs && unit.debuffs.weak) {
                    ...
                    if (unit.debuffs && unit.debuffs.weak) speed *= 0.5; // WarCry effect

                    // Shield Wall - Visual only? No, damage reduction handled in Attack
